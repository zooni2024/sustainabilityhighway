...
export default async function TopicDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // Note: Data file uses "Energy" but slug "energy".
    // The guide frontmatter uses "Energy".
    // So we match `p.meta.topic === topic.name`

    const posts = getAllPosts().filter(p => p.meta.topic === topic.name);

    return (
        <div className="container mx-auto px-4 py-12">
            <h1 className="text-3xl font-bold text-ksa-green-900 mb-8">{topic.name}</h1>
            ...
            {posts.length > 0 ? (
                ...
                <p className="text-gray-500 italic">No guides found for this topic yet.</p>
            )}
        </div>
    );
}
